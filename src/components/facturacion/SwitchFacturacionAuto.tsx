...
export function SwitchFacturacionAuto() {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-1.5 rounded-md border bg-card px-2 py-1 text-xs text-muted-foreground">
    ...
    <div className="relative">
      <div className={`flex items-center gap-2 rounded-md border px-2 py-1 transition-colors ${
        ...
        <button
          ...
          className="shrink-0 rounded p-0.5 text-muted-foreground hover:bg-muted transition-colors"
          title="Ver detalles"
        >
      ...
      {abierto && (
        <div className="absolute right-0 top-full z-40 mt-1.5 w-[22rem] rounded-lg border bg-card p-3 shadow-lg space-y-2.5">
            ...
            <div className="rounded-md border border-amber-300 bg-amber-50 p-2 space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={saving}
                  className="flex items-center gap-1.5 rounded-md bg-amber-600 px-2 py-1 text-[11px] font-semibold text-white hover:bg-amber-700 disabled:opacity-50 transition-colors"
                >
          ...
          <div className="space-y-1 border-t pt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                min={0}
                step={1000}
                className="h-6 flex-1 rounded border px-1.5 text-xs bg-background"
              />
              {Number(montoInput) !== config.facturacion_auto_monto_max && (
                <button
                  onClick={() => guardar({ facturacion_auto_monto_max: Number(montoInput) || 100000 })}
                  disabled={saving}
                  className="h-6 rounded bg-primary px-2 text-[11px] font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors"
                >
          ...
          {/* Última corrida */}
          <div className="space-y-1 border-t pt-2">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Última corrida</p>
            {config.facturacion_auto_ultimo_run ? (
              <>
                ...
                {config.facturacion_auto_ultimo_error && (
                  <p className="rounded-md border border-red-200 bg-red-50 px-1.5 py-1 text-[10px] text-red-700 leading-snug">
                    <strong>Se frenó por un error:</strong> {config.facturacion_auto_ultimo_error}
                  </p>
                )}
              </>
            ) : (
              <p className="text-[10px] text-muted-foreground">Todavía no corrió.</p>
            )}
          </div>
        </div>
      )}
